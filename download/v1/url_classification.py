...
class URLClassification:
    ...
    def get_filename_from_url(self, url: str) -> str:
        ...
        try:
            ...
            # 使用DatabaseManager检查文件名重复
            while self.db_manager.is_filename_exists(f"{name}{ext}"):
            ...
            error_msg = f"处理文件名时出错: {str(e)}"
            self.db_manager.add_error_task(url, "unnamed_file", error_msg, 'v1')
            return "unnamed_file"
    ...
    def classify_url(self, url: str):
        ...
        try:
            ...
            if not is_valid:
                self.db_manager.add_error_task(url, "invalid_url", result, 'v1')
            ...
            # 检查文件是否已存在
            if self.db_manager.check_file_exists_in_all_tables(file_name) == False:
                QMessageBox.warning(None, "文件已存在", 
                    f"文件 {file_name} 已存在于下载队列或已完成列表中")
                return
            ...
            # 获取当前下载中的任务数
            downloading_count = self.db_manager.get_downloading_count()

            task_data = {
                'url': url,
                'file_name': file_name,
                'status': '下载中' if downloading_count < max_tasks else '等待中',
                'time': current_time
            }

            if downloading_count < max_tasks:
                # 使用DatabaseManager添加下载任务
                self.db_manager.add_downloading_task(task_data)
                
                # 调用FirstLevelThread管理器
                from download.v2.first_level_thread import FirstLevelThread
                FirstLevelThread.manage_threads(max_tasks)
            else:
                # 使用DatabaseManager添加等待任务
                self.db_manager.add_waiting_task(task_data)
 
        except Exception as e:
            error_msg = f"URL处理失败: {str(e)}"
            self.db_manager.add_error_task(url, "error_url", error_msg, 'v1')
