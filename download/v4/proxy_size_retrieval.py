...
class ProxySizeRetrieval:
    def __init__(self, thread_id: int, file_name: str):
        ...
        # 记录初始化日志
        self.log_message(f"初始化 ProxySizeRetrieval - 线程ID: {thread_id}, 文件名: {file_name}")
    ...
    def _handle_failure(self, url: str, error_reason: str):
        ...
        # 获取当前任务信息
        task = self.db_manager.get_task_by_url(url)
        if task:
            # 添加错误记录
            self.db_manager.add_error_task(
                url=url,
                file_name=self.file_name,  # 使用实例的 file_name
                error_msg="代理服务器故障，请检查代理，或许是资源不允许代理",
                sign='v4'
            )
    ...
    def _call_determination_of_threaquantity(self, url: str, file_size: int):
        ...
        try:
            ...
            thread_determination = ThreadDetermination(
                ...
                url=url,
                file_name=self.file_name,  # 传递文件名
                file_size=file_size
            )
