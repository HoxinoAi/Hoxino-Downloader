...
class MainUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # 创建进度条
        self.progress_bar = ProgressBar(self)  # 添加 self 作为父对象
    ...
    def init_ui(self):
        ...
        self.email_label.setFont(QFont("楷体", 10))  # 修改为楷体
        ...
        for name, table in self.lists.items():
            ...
            if name == "下载中":
                ...
                table.row_selected.connect(
                    self.on_row_selected,  # 先连接到中间处理函数
    ...
    def load_downloading_tasks(self, table):
        ...
        # 从数据库读取下载中的任务
        tasks = self.db_manager.get_downloading_tasks()
    ...
    def create_table(self, tab_name=None):
        """创建下载列表"""
        table = DownloadList()
        table.tab_name = tab_name  # 直接设置属性
    ...
    def on_row_selected(self, thread_id: int):
        ...
        try:
            ...
            if thread_id == -1:
                 # 通知进度条取消选择并停止检查
                self.progress_bar.on_task_selected(-1, "")  # 传递-1和空文件名
                self.progress_bar.hide()  # 隐藏进度条
                return
            
            # 获取任务信息
            task = self.db_manager.get_task_by_thread_id(thread_id)
            if task and 'file_name' in task:
                # 显示进度条容器
                self.progress_bar.show()
                # 传递线程ID和文件名
                self.progress_bar.on_task_selected(thread_id, task['file_name'])
            else:
    ...
    def update_task_list(self):
        ...
        tasks = db_manager.get_downloading_task_details()
        ...
        for task in tasks:
            ...
            self.setItem(row, 1, QTableWidgetItem(task['url']))
            self.setItem(row, 2, QTableWidgetItem(task['status']))
            ...
            # 设置时间
            time_item = QTableWidgetItem(task['time'])
            self.setItem(row, 5, time_item)
            
            # 设置线程数
            threads_item = QTableWidgetItem(str(task.get('threads', 0)))
            self.setItem(row, 6, threads_item)
            
            # 存储线程ID用于后续操作
            self.setItem(row, 7, QTableWidgetItem(str(task.get('thread_id', ''))))
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            # 从数据库获取正在下载的任务
            db_manager = DatabaseManager()
            active_tasks = db_manager.get_downloading_task_details()
            active_tasks = [task for task in active_tasks if task.get('status') == '下载中']
